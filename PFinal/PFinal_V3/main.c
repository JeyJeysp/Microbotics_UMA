...
uint32_t g_ui32CPUUsage, g_ulSystemClock, contSharp = 0, umbralSharp = 15;
    ...
    while(1)
    {
        ...
        switch(modo)
        {
            case BUSQUEDA:
            {
                ...
                // Cuando detecte la interrupcion del sharp o whisker pasa al modo ataque
                ...
                if (xQueueReceive(colaMovimiento, &mov_recibido, 0) == pdTRUE)
                {
                    ...
                    if (mov_recibido.t == 'c')
                    {
                        if (mov_recibido.v >= 0)
                        {
                            mov_rueda_universal(RUEDA_IZQUIERDA, 100);
                            mov_rueda_universal(RUEDA_DERECHA, 90);
                            objetivo_mov = round(mov_recibido.v/3);
                        }
                    ...
                    else if (mov_recibido.t == 'g')
                    {
                        if (mov_recibido.v >= 0)
                        {
                            mov_rueda_universal(RUEDA_IZQUIERDA, 100);
                            mov_rueda_universal(RUEDA_DERECHA, -90);
                            objetivo_mov = round(mov_recibido.v/30);
                        }
                        else if (mov_recibido.v < 0)
                        {
                            mov_rueda_universal(RUEDA_IZQUIERDA, -100);
...
int main(void)
{
    ...
    // TAREA DEL SHARP
    if ((xTaskCreate(TareaADC0Sharp, "TaskADC0", ADCTASKSIZE, NULL, tskIDLE_PRIORITY + ADCTASKPRIO, NULL) != pdTRUE))
    {
        while (1);
    }
...
//! GPIOInt_A_Handler
//!
//! Manejador del pin A para las interrupciones del whisker y Sharp.
//!
//! Tipo: ISR
//
//*****************************************************************************
...
//! GPIOInt_C_Handler
//!
//! Manejador del pin C para las interrupciones de los sensores de linea.
//!
//! Tipo: ISR
//
//*****************************************************************************
...
//! GPIOInt_F_Handler
//!
//! Manejador del pin F para el boton derecho (inicio del modo de busqueda).
//!
//! Tipo: ISR
//
//*****************************************************************************
