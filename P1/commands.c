...
//  Ciclos para amplitud de pulso de parada a 1.55ms (para que esté parado el motor derecho)
#define STOPCOUNT_DER ((SysCtlClockGet() / 16) * 0.00155)
//  Ciclos para amplitud de pulso de parada a 1.55ms (para que esté parado el motor izquierdo)
#define STOPCOUNT_IZQ ((SysCtlClockGet() / 16) * 0.00155)
...
static Cmd_parar_motor(int argc, char *argv[])
{
    //  DBC: Cuando se han pulsado los 2 botones vamos a parar el motor. Para pruebas
...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "parar_motor", Cmd_parar_motor, "\t\t: Parar motor" },
#if ( configUSE_TRACE_FACILITY == 1 )
