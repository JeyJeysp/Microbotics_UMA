...
//Esta funcion obtiene el campo "tipo comando" de la trama
uint8_t decode_command_type(uint8_t * buffer)
{
	return buffer[0];
}

//Esta funcion extrae el parametro de la trama y comprueba que el tamanio sea correcto
//ptrtoparam es el puntero a la zona de memoria donde esta el parametro
//param_size es su tamanio
//param es una estructura por REFERENCIA
//payload es el tamanio de la estructura que se pasa por REFERENCIA
//Devuelve: Tamanio del parametro recibido o un valor negativo si hay error
...
//Esta funcion obtiene un puntero a la zona de memoria donde esta el par�metro dentro de la trama
//buffer es la zona de memoria donde esta almacenada la trama "Desestufada"
//frame_size es su tamanio
//campo es un puntero a void que se pasa por REFERENCIA. Queda apuntando a la zona de memoria donde esta el parametro
//Devuelve: Tamanio del parametro recibido o un valor negativo si hay error
